...
extension StructuredText {
  ...
  fileprivate struct BlockLayoutView<Content: View>: View {
    ...
    var body: some View {
      // List spacing lives in the environment, so it is known during this
      // body evaluation. Using it directly means the first layout pass
      // already has the right gaps; waiting for the preference (as we
      // still must for paragraph/heading spacing) is a pass late.
      // TEEMOON PATCH — see ../../../../VENDORING.md.
      let spacing = listItemSpacingEnabled ? resolvedListItemSpacing : blockSpacing
      content
        ...
        .layoutValue(key: BlockSpacingKey.self, value: spacing)
    }
  }
