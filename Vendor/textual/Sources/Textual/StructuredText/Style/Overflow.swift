...
public struct Overflow<Content: View>: View {
  ...
  public var body: some View {
    ...
    case .scroll:
      ...
      // TEEMOON PATCH — see ../../../../VENDORING.md. A `ScrollView` is greedy
      // in BOTH axes, so this one's height came only from `contentHeight`
      // above: `@State` written by `.onGeometryChange`, which lands a pass
      // LATE. Every code block and every table therefore measured near-zero
      // first and its real height a frame later — a resize per block per
      // appearance, which in a recycling transcript is the scroll jank.
      // Fixing the size vertically makes the scroll view adopt its content's
      // IDEAL height in the same pass, so the first measurement is already
      // right; `contentHeight` stays as the mechanism for later changes.
