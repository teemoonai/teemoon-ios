...
public struct StructuredText: View {
  // TEEMOON PATCH 2/2 (see ../../../VENDORING.md) — PARSE BEFORE THE FIRST
  // MEASURE, NOT AFTER IT.
  //
  // Upstream holds the parsed string in `@State` seeded from
  // `.onChange(initial: true)`, which runs when the view appears — i.e. AFTER
  // the layout system has already asked this view how big it is. The first
  // measurement therefore describes an EMPTY document, and the real height
  // arrives a frame later as a resize. In a scrolling list that is a resize per
  // row per appearance: a 3,801pt reply first measured 83pt, painted over the
  // row below it, and jumped when the parse landed.
  //
  // Here `resolved` parses during body evaluation, so the very first size this
  // view reports is its real one. `RenderMemo` keeps the result against the
  // markup it came from, so a recycled view can never show the previous row's
  // text and nothing is parsed twice; teemoon additionally passes a memoizing
  // parser, which is what makes re-entry — a cell scrolling back into view —
  // free even across view instances.
  @State private var memo = RenderMemo<String, AttributedString>()

  private var resolved: AttributedString {
    memo.value(for: markup) { (try? parser.attributedString(for: $0)) ?? .init() }
  }
