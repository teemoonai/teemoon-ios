...
public actor Engine {
  ...
  ///
  /// teemoon change: `private` -> internal, so `Engine+RawAudio.swift` can reach the
  /// low-level input-data API this package otherwise never calls. Revert if upstream
  /// ships an audio path of its own.
