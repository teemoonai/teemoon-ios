...
public protocol Tool: Decodable {
  ...
  /// The JSON schema describing this tool to the model.
  ///
  /// LOCAL PATCH (teemoon): this was declared ONLY in the protocol extension
  /// below. A conformer that defined its own `getSchema()` therefore never had
  /// it called — `ToolManager` iterates `[Tool]` existentials, and a method that
  /// is not a protocol requirement dispatches statically to the extension.
  ///
  /// The failure is silent and expensive: teemoon's shim holds the model's
  /// arguments untyped (so a small model's argument shape can be repaired
  /// rather than rejected) and supplies the real schema by overriding this. With
  /// the override never invoked, the default reflected over `@ToolParam`
  /// properties, found none, omitted `parameters` altogether — and the model,
  /// told the tool takes no arguments, emitted `"arguments":{}` on every call.
  /// `web_search` ran and searched for nothing.
  ///
  /// Declaring it here makes the extension a default implementation rather than
  /// the only implementation. Upstream behaviour is unchanged for conformers
